...
// XML Payload to fetch Debit Note vouchers for a specific date range
const DEBIT_NOTE_XML = `
<ENVELOPE>
  <HEADER>
    <VERSION>1</VERSION>
    <TALLYREQUEST>EXPORT</TALLYREQUEST>
    <TYPE>COLLECTION</TYPE>
    <ID>DebitNoteCollection</ID>
  </HEADER>
  <BODY>
    <DESC>
      <STATICVARIABLES>
      ...
      <TDL>
        <TDLMESSAGE>
          <COLLECTION NAME="DebitNoteCollection" ISINITIALIZE="Yes">
            <TYPE>Voucher</TYPE>
            <FETCH>Date</FETCH>
            ...
            <FETCH>ORIGINALVOUCHERNO</FETCH>
            <FETCH>ORIGINALVOUCHERDATE</FETCH>
...
/**
 * Parses the raw XML response from Tally into a structured JSON array of Debit Notes.
 * @param {string} xmlResponse 
 * @param {object} selectedCompany 
 */
function parseDebitNoteVouchers(xmlResponse, selectedCompany) {
  const vouchers = [];
  ...
  while ((match = vchRegex.exec(xmlResponse)) !== null) {
    ...
    // Some debit notes use ORIGINALINVOICENO, some use BASICBUYERDOCUMENTNO or REFERENCE
    const origInvoiceNo = extract('ORIGINALINVOICENO', vContent) || extract('ORIGINALVOUCHERNO', vContent) || extract('REFERENCE', vContent) || extract('BASICBUYERDOCUMENTNO', vContent) || "";
    const origInvoiceDate = extract('ORIGINALINVOICEDATE', vContent) || extract('ORIGINALVOUCHERDATE', vContent) || extract('REFERENCEDATE', vContent) || extract('BASICBUYERDOCUMENTDATE', vContent) || "";
    ...
    while ((invMatch = invRegex.exec(vContent)) !== null) {
      ...
      let hsn = extract('HSNCODE', invContent) || extract('GSTHSNNAME', invContent);

      let rate = parseFloat(extract('RATE', invContent).replace(/[^\d\.]/g, '') || 0);
      let amount = parseFloat(extract('AMOUNT', invContent).replace(/[^\d\.-]/g, '') || 0);
      amount = Math.abs(amount); // Tally uses negative for credits
    ...
    // Tally accounting: Sum of debits (is_debit = true) is usually the total invoice amount for sales/debit notes
    const totalAmount = ledgers.filter(l => l.is_debit).reduce((acc, l) => acc + Math.abs(l.amount), 0);
    ...
    const doc = {
      ...
      other_details: {
        ...
        destination: extract('BASICFINALDESTINATION', vContent) || extract('DESTINATION', vContent) || extract('BASICPLACEBEFOREDELIVERY', vContent)
      },
      items,
      ledgers,
      totals: {
...
async function exportDebitNoteAsPdf(voucher, companyObj, bankObj) {
    ...
    taxLedgers.forEach(tl => {
      ...
      tableRowsHtml += `
        <tr class="tax-row">
          ...
          <td class="c-rate"></td>
          <td class="c-per"></td>
          <td class="c-amt">${formatAmount(taxAmt)}</td>
        </tr>
      `;
    });
  ...
  const roundoffLedger = (voucher.ledgers || []).find(le => le.ledger_name.toUpperCase().includes('ROUND') || le.ledger_name.toUpperCase().includes('SHORT'));
    if (roundoffLedger) {
        const roAmount = roundoffLedger.amount;
        grandTotal += roAmount;
        tableRowsHtml += `
        <tr class="tax-row">
          <td class="c-sl"></td>
  ...
  const htmlTemplatePath = path.join(__dirname, 'html', 'debit_note.html');
  const cssPath = path.join(__dirname, 'css', 'debit_note.css');
  ...
  if (success) {
    ...
    console.error(`[Debit Note PDF Export] Error for ${voucher.voucher_info.voucher_number}`);
    return null;
  }
}
