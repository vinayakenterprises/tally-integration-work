...
} = require('./tallyCreditNoteCore.cjs');
...
/**
 * Main routine: Fetch Credit Notes, generate PDFs, and push to API.
 */
async function syncAndPushCreditNotes(queryDate) {
  ...
  try {
    ...
    if (!selectedCompany) {
      console.error('[Credit Note Sync Error] Could not determine Tally company or no company open.');
      return { status: 'error', reason: 'no_company' };
    }

    console.log(`[Credit Note Sync] Using Company: ${selectedCompany.name}`);

    // 2. Fetch Credit Notes
    const xmlPayload = injectCompanyInXml(CREDIT_NOTE_XML, selectedCompany.name, queryDate, queryDate);

    console.log(`[Credit Note Sync] Fetching Credit Notes for ${displayDate}...`);
    const xmlResponse = await fetchFromTallyWithRetry(xmlPayload);

    // 3. Parse Credit Notes
    const vouchers = parseCreditNoteVouchers(xmlResponse, selectedCompany);
    ...
    if (previous && previous.hash === newHash) {
      console.log(`[Credit Note Sync] No changes detected for ${displayDate}. Skipping push.`);
      return { status: 'unchanged', date: displayDate };
    }

    console.log(`[Credit Note Sync] Data changed (or first run) for ${displayDate}. Generating PDFs and pushing to API...`);
    ...
    try {
      ...
      console.warn(`[Credit Note PDF Export] Could not fetch bank ledgers: ${err.message}`);
    }

    // 5. Generate PDFs for all Credit Notes locally
    for (const v of vouchers) {
      console.log(`[Credit Note PDF Export] Generating PDF for Credit Note: ${v.voucher_info.voucher_number}...`);
      const pdfPath = await exportCreditNoteAsPdf(v, selectedCompany, bankDetails);
      if (pdfPath) {
    ...
    for (const cn of vouchers) {
      try {
        console.log(`[Credit Note API] Pushing Credit Note ${cn.voucher_info.voucher_number}...`);

        // Skip actual push if dummy URL is provided
        if (PUSH_API_URL === 'abc-credit-note' || !PUSH_API_URL.startsWith('http')) {
          console.log(`[Credit Note API] Dummy URL configured ('${PUSH_API_URL}'). Simulating success.`);
          anySuccess = true;
          continue;
        }
        ...
        form.append('document_type', 'Credit Note');
        ...
        if (pushResponse && pushResponse.status >= 200 && pushResponse.status < 300) {
          console.log(`[Credit Note API] Success! Status: ${pushResponse.status} OK`);
          anySuccess = true;
        } else {
          allSuccess = false;
          console.error(`[Credit Note API] Non-success response for ${cn.voucher_info.voucher_number}`);
        }
      } catch (err) {
        allSuccess = false;
        console.error(`[Credit Note Sync Error] Push failed for ${cn.voucher_info.voucher_number}: ${err.message}`);
      }
    }
    ...
    console.error(`[Credit Note Sync Error] Sync failed: ${err.message}`);
...
function runScheduler() {
  console.log(`[Credit Note Scheduler] Started. Polling every ${SCHEDULE_INTERVAL_MS / 1000}s`);

  const tick = async () => {
    if (!isWithinScheduleWindow()) {
      console.log('[Credit Note Scheduler] Outside active window. Sleeping...');
      return;
    }

    try {
      const today = getEffectiveQueryDate();
      console.log(`\n[Credit Note Scheduler] Running sync for ${today} at ${new Date().toLocaleTimeString()}...`);
      await syncAndPushCreditNotes(today);
    } catch (e) {
      console.error('[Credit Note Scheduler] Unhandled error during poll:', e);
    }
  };
...
if (require.main === module) {
  ...
  if (args[0] === 'daemon' || args[0] === 'serve' || args[0] === 'schedule') {
    ...
    console.log(`[Credit Note Sync] Running one-time sync for custom date: ${cliDate}`);
    syncAndPushCreditNotes(cliDate).then((outcome) => {
      console.log(`[Credit Note Sync] One-time sync completed. Status: ${outcome.status}`);
...
module.exports = { syncAndPushCreditNotes };
